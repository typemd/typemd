...
| File | Role |
|------|------|
| `display.go` | DisplayProperty struct (includes DateFormat/DatetimeFormat for configurable display) + FormatValue/Format rendering + DefaultDateFormat/DefaultDatetimeFormat constants + Vault.BuildDisplayProperties facade (injects format config) |
...
| `prop_editor.go` | Property editor sub-model (propEditor): cursor navigation, editable property filtering, textinput/select/multi_select/checkbox/date edit widgets, Render with cursor indicator |
