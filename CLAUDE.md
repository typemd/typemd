...
| `view_mode_cell_edit.go` | Table view inline cell editing: cellEdit struct, activateCellEdit/cancelCellEdit/applyCellValue, updateCellEdit key handling, isCellReadOnly detection, viewCellToastMsg/viewCellSavedMsg messages |
...
The TUI supports **inline property editing** via the `propEditor` sub-model (`tui/prop_editor.go`, `prop_editor_update.go`, `relation_picker.go`). When the properties panel is focused (Tab from body), a cursor (▸) highlights the current editable property. Navigation uses j/k; Enter activates type-appropriate editing; Esc returns to sidebar. Read-only properties (created_at, updated_at, reverse relations, backlinks) are displayed but skipped during navigation. Edit widgets by property type: **string/number/date/datetime/url** use a textinput with type-specific validation (via `core.ValidatePropertyValue`); **checkbox** toggles directly between ☐/☑ on Enter/Space; **select** opens an option list picker; **multi_select** opens a multi-picker with Space toggle and Enter confirm; **relation** (single-value) opens a searchable relation picker with fuzzy filtering and a "(none)" option to clear; **relation** (multi-value, including `tags`) opens a multi-select relation picker with Space toggle, Enter confirm, and fuzzy search. Relation editing uses `Vault.LinkObjects`/`UnlinkObjects` for mutations, handling bidirectional relations automatically. Validation errors appear as toast notifications. Edits auto-save on confirm. The help bar shows `[PROPS]` during navigation, `[EDIT]` during textinput editing, and `[PICK]` during picker selection (select, multi_select, relation). The properties panel border changes to edit color (orange) during active editing. Locked objects (with `locked: true` in frontmatter) display a 🔒 badge right-aligned in the title panel. Editing is blocked for locked objects — pressing Tab (props), `e` (body edit), or `r` (rename) shows a toast notification instead. Pressing `L` (uppercase) toggles the lock state of the selected object.
