...
- Type schemas: `.typemd/types/<name>/schema.yaml`. Reserved system properties (`name`, `description`, `created_at`, `updated_at`, `tags`, `locked`, `archived`) cannot be redefined; `name` can appear with only a `template` field. Optional schema fields: `plural`, `emoji`, `unique`, `version`, `color`, `description`.
