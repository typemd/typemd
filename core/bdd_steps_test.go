...
func (dc *domainContext) aTypeSchemaWithAnEnumPropertyMissingValues(typeName string) {
	schema := fmt.Sprintf("name: %s\nproperties:\n  - name: status\n    type: enum\n", typeName)
	os.WriteFile(filepath.Join(dc.vault.TypesDir(), typeName+".yaml"), []byte(schema), 0644)
}
...
func initDomainSteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^a type schema "([^"]*)" with an enum property missing values$`, dc.aTypeSchemaWithAnEnumPropertyMissingValues)
