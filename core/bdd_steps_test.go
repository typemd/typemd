...
func (dc *domainContext) iUnlinkFromViaWithoutBothFlag(sourceName, targetName, relation string) {
	...
	dc.lastErr = dc.vault.UnlinkObjects(source.ID, relation, target.ID, false)
}
...
func (dc *domainContext) aBookObjectExistsWithTagReferenceByName(bookName, tagName string) {
	...
	book.Properties[TagsProperty] = []any{"tag/" + tagName}
	dc.vault.SaveObject(book)
	dc.objects[bookName] = book
	dc.currentObject = book
}
