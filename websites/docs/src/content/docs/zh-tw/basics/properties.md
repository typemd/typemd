...
每個 Object 都支援八個由 TypeMD 管理的系統屬性。這些屬性提供每個知識管理工具都需要的基礎中繼資料——身份識別、描述、時間追蹤、分類、別名、保護和歸檔——使用者不需要在每個 type schema 中自行定義。並非所有系統屬性都會出現在每個 Object 的 frontmatter 中：`name` 會在建立時自動填入，`created_at` 和 `updated_at` 在使用 CLI 時設定，而 `description`、`tags`、`aliases`、`locked` 和 `archived` 只在使用者明確設定後才會出現。

| 屬性 | 說明 | 可變性 | 為什麼需要 |
|------|------|--------|-----------|
| `name` | 顯示名稱，建立時從 slug 自動填入。缺少或為空時，會 fallback 到由檔名衍生的 [Display ID](/zh-tw/concepts/glossary#display-id)。 | 使用者撰寫 | 將顯示名稱與檔名解耦——支援空格、大小寫和重新命名而不需搬動檔案 |
...
| `aliases` | 選填的替代名稱列表，用於 wiki-link 解析 | 使用者撰寫 | 讓 Object 可以透過任何已知名稱被引用——翻譯、縮寫或舊名稱 |
| `locked` | 鎖定物件，防止編輯 | 使用者撰寫 | 保護已完成或歸檔的 Object 不被意外修改 |
| `archived` | 軟刪除標記——從預設查詢中隱藏物件 | 使用者撰寫 | 讓不再活躍的 Object 退場而不永久刪除 |

**使用者撰寫**的屬性（`name`、`description`、`tags`、`aliases`、`locked`、`archived`）可以被 [object template](/zh-tw/basics/templates) 覆蓋。**自動管理**的屬性（`created_at`、`updated_at`）無法被覆蓋——它們永遠反映實際的建立和修改時間。
...
除了上述八個儲存的系統屬性外，TypeMD 還提供在執行時期解析的非儲存屬性——它們不會出現在 YAML frontmatter 中。
...
| `pin` | integer | 正整數，讓屬性突出顯示在 TUI 屬性面板頂端。數值越小排越前面。釘選屬性排在未釘選屬性之前，並以水平分隔線區隔。 |
| `default` | any | 建立新 object 時指定的預設值 |

**為什麼需要 emoji？** 在緊湊的 UI 環境中（TUI 屬性面板、表格欄位），屬性名稱會佔用大量水平空間。Emoji 提供空間效率高的視覺識別，一眼就能辨認。

**為什麼需要 pin？** 沒有置頂功能時，所有屬性在 Properties 面板中平等顯示。使用者必須掃描整個列表才能找到 status 或 rating 等關鍵中繼資料。置頂讓 type 作者標記特定屬性，突出顯示在 Properties 面板頂端，並以分隔線與其餘屬性區隔。
...
如果同一個屬性出現在多個 type 中（例如 `due_date` 同時在 `project` 和 `task` 裡），你可以在 `properties/` 目錄中定義一次，然後用 `use` 關鍵字引用。

### 定義共用屬性

每個共用屬性是 `properties/<name>.yaml` 下的獨立檔案，其中 `<name>` 就是屬性名稱。不需要外層包裝鍵——檔案直接包含屬性定義。
...
每個檔案支援與 type schema 屬性相同的所有欄位：`type`、`emoji`、`pin`、`options`、`target`、`default`、`multiple`、`bidirectional` 和 `inverse`。在 type schema 中透過 `use:` 引用時，只有 `pin`、`emoji` 和 `description` 可以被覆寫——其他欄位皆繼承自共用屬性定義。

`properties/` 目錄為選填。如果該目錄不存在或沒有任何 `.yaml` 檔案，TypeMD 會將其視為空的共用屬性集合。
...
| | 共用屬性 | 內嵌屬性 |
|---|---------|---------|
| 定義位置 | `properties/<name>.yaml` | `types/<type>/schema.yaml` |
